...
crop_grange = function(df, range = NULL, noise_std = 0.03){
  if(!is.null(range)){
    upper = range[2]
    lower = range[1]
    cropFun = plyr::colwise(function(colData){
      colData[colData > upper] = upper + stats::rnorm(sum(colData > upper), 0, noise_std)
      colData[colData < lower] = lower + stats::rnorm(sum(colData < lower), 0, noise_std)
      return(colData)
    })
