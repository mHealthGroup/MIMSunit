...
error_data = adply(settings, 1, function(setting) {
  ...
  error = counts %>% ddply(c("RPM"), function(counts) {
    ...
    mse_counts = counts %>% dplyr::filter(DEVICE != "TAS1C32140067" & DEVICE != "ActivPal3")
...
error_data_melted = error_data_merged %>% filter(!name == "Proposed with narrower passband (0.25-2.5Hz)\n and without extrapolation, resample to 10Hz") %>% melt(c("RPM", "name"))
