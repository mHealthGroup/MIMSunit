...
```{r, include=FALSE}
acc_factor = 60 / 5
filtered_df = df %>% dplyr::filter(DEVICE != "Activpal")
summary_df = filtered_df %>% ddply(
  c("TYPE", "HZ"),
  summarise,
  N = length(VALUE),
  mean = mean(VALUE),
...
summary_df_melted = summary_df[c('TYPE','HZ','cv')] %>% reshape2::melt(c("HZ", "TYPE"))
current_run = format(Sys.time(), "%Y%m%d%H")
if(stringr::str_detect(wd, "articles")){
  write.csv(summary_df_melted, file = paste0("../../inst/table/shaker_consistency_cv", current_run ,".csv"), quote = FALSE, row.names = FALSE)
}else{
  write.csv(summary_df_melted, file = paste0("../inst/table/shaker_consistency_cv", current_run ,".csv"), quote = FALSE, row.names = FALSE)
}
...
current_run = format(Sys.time(), "%Y%m%d%H")
if(stringr::str_detect(wd, "articles")){
  write.csv(summary_df, file = paste0("../../inst/table/shaker_consistency_detail_", current_run,".csv"), quote = FALSE, row.names = FALSE)
}else{
  write.csv(summary_df, file = paste0("../inst/table/shaker_consistency_detail_", current_run,".csv"), quote = FALSE, row.names = FALSE)
}
...
p2 = ggplot(data = summary_df, aes(
  ...
  geom_line(size=1.2) + geom_point(aes(shape = NAME), size = 3) +
  scale_colour_discrete(name = "Device") +
  scale_shape_discrete(name = "Device") +
